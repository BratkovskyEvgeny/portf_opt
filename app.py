...
def layout_plots(sharpe, stocks, ratio) -> None:
    ...
    with col2:
        if st.button("Year", key="portfolio1"):
            st.session_state.window_size = 360
    with col3:
        if st.button("Quarter", key="portfolio2"):
            st.session_state.window_size = 90
    with col4:
        if st.button("Month", key="portfolio3"):
    ...
    for ticker, detail in zip(ratio.ticker.values, ratio.detail.values):
        ...
        with col1:
            if st.button("3Year", key=f"{ticker}1"):
                st.session_state.window_size = 1080
        with col2:
            if st.button("Year", key=f"{ticker}2"):
                st.session_state.window_size = 360
        with col3:
            if st.button("Quarter", key=f"{ticker}3"):
                st.session_state.window_size = 90
        with col4:
            if st.button("Month", key=f"{ticker}4"):
                st.session_state.window_size = 30
        plot_stock_data(
            df=stocks,
            window=st.session_state.window_size,
